  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      ...
      <p className="text-muted text-sm mb-8">
        {q ? <>{t("Results for")} <span className="text-text font-bold">"{q}"</span> {t("— searched towers, devices, zones & IP allocations")}</>
      ...
      {total > 0 && <p className="text-muted2 text-xs mt-8">{t(total === 1 ? "{n} result" : "{n} results", { n: total })}</p>}
    </main>
  );
}
